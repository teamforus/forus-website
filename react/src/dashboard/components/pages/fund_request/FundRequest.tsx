...
export default function FundRequest() {
    ...
    const fetchEmployees = useCallback(() => {
        ...
        employeeService
            ...
            .then(
                ...
                (res) => pushDanger('Error', res?.data?.message || 'Unknwon error.'),
            )
    ...
    const fetchFundRequest = useCallback(() => {
        ...
        fundRequestService
            ...
            .then(
                (res) => setFundRequest(mapRequestFlags(res.data.data)),
                (res) => pushDanger('Error', res?.data?.message || 'Unknwon error.'),
            )
            .finally(() => setProgress(100));
    ...
    const assignRequest = useCallback(
        () =>
            fundRequestService.assign(activeOrganization?.id, fundRequest.id).then(
                () => {
    ...
    const requestResignAllEmployeesAsSupervisor = useCallback(() => {
        fundRequestService.resignAllEmployeesAsSupervisor(activeOrganization.id, fundRequest.id).then(
            () => {
    ...
    const requestResign = useCallback(() => {
        ...
        fundRequestService.resign(activeOrganization.id, fundRequest.id).then(
            () => {
                pushSuccess('Gelukt!', 'U heeft zich afgemeld van deze aanvraag.');
                reloadRequest();
            },
