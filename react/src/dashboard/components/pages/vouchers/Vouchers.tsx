...
export default function Vouchers() {
    ...
    const [fundsById, setFundsById] = useState<{ [key: number]: Fund }>(null);
    ...
    const showTableTooltip = useCallback(
        (tooltipKey) => {
            ...
            if (showTableConfig && tableConfigCategory == 'tooltips') {
                ...
                setToast(
                    ['Klik op het informatie icoon rechtsboven in de tabel voor meer uitleg over deze pagina.'].join(
                        '',
                    ),
                );
            }
    ...
    useEffect(() => {
        const columnsData = voucherService.getColumns().filter((column) => {
            if (!filter.activeValues.fund_id || !column.fundType) {
                return true;
            }

            return fundsById[filter.activeValues.fund_id].type == column.fundType;
        });

        setColumns(columnsData);
        setColumnKeys(columnsData.map((column) => column.key));
        setTooltips(columnsData.filter((column) => column.tooltip).reduce((val, item) => [...val, item.tooltip], []));
    }, [filter.activeValues.fund_id, fundsById, voucherService]);
                                        ...
                                        {filter.show && (
                                            <div className="inline-filters-dropdown-content">
                                                ...
                                                <div className="form">
                                                    ...
                                                    <FilterItemToggle label={t('vouchers.labels.amount_available')}>
                                                        <div className="row">
                                                            <div className="col col-sm-6">
                                                                <input
                                                                    type="number"
                                                                    ...
                                                                    onChange={(e) =>
                                                                        filter.update({
                                                                            amount_available_min: e.target.value,
                                                                        })
                                                                    }
                                                                />
                                                            </div>
                                                            ...
                                                            <div className="col col-sm-6">
                                                                <input
                                                                    ...
                                                                    onChange={(e) =>
                                                                        filter.update({
                                                                            amount_available_max: e.target.value,
                                                                        })
                                                                    }
                                                                />
                                                            </div>
