...
export default function useFilters<T = FilterModel>(
    ...
    const getTimeout = useCallback(
        (current, old) => {
            return throttledValues.filter((filter) => current[filter] !== old[filter]).length > 0 ? 1000 : 0;
        },
        [throttledValues],
    );
    ...
    return { show, setShow, values, activeValues, update, resetFilters };
}
