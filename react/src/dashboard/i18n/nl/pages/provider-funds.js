export default {
    title: {
        available: 'Beschikbare fondsen',
        pending_rejected: 'Aanmeldingen fondsen',
        active: 'Actieve fondsen',
        archived: 'Archief',
        invitations: 'Uitnodigingen',
        invitations_archived: 'Gearchiveerde uitnodigingen',
        expired_closed: 'Archief',
        main: 'Fondsen',
    },
    buttons: {
        join: 'Aanmelden',
        clear_filter: 'Wis filter',
        unsubscribe: 'Afmelden',
    },
    labels: {
        categories: 'Categorieën',
        nocategories: 'Geen categorieën',
        date: 'Begindatum / Einddatum',
        max_amount: 'Maximaal', // "Maximaal tegoed per voucher",
        closed: 'Gesloten',
        accept_invitation: 'Accepteren',
        allow_budget: 'Budget',
        allow_products: 'Producten',
        allow_some_products: 'Scan specifiek aanbod',
        view_products: 'Bekijk aanbod',
        cancel_application: 'Annuleren',
        unsubscribe: 'Afmelden',
        fund: 'Fonds',
        organization: 'Organisatie',
        tag: 'Label',
        start_date: 'Begindatum',
        end_date: 'Einddatum',
        actions: 'Acties',
        status: 'Status',
    },
    empty_block: {
        available: 'Er zijn geen beschikbare fondsen waar u zich voor kunt aanmelden.',
        active: 'Er zijn geen fondsen waar u actief voor bent.',
        archived: 'Er zijn geen gearchiveerde fondsen.',
        invitations: 'Er zijn geen openstaande uitnodigingen die u kunt accepteren.',
        invitations_archived: 'Er zijn geen verlopen uitnodigingen.',
        pending_rejected: 'Er zijn geen fondsen waar u aanmeldingen voor hebt.',
        unsubscriptions: 'Er zijn geen afmeldingsaanvragen gevonden.',
    },
    filters: {
        labels: {
            search: 'Zoeken',
            organizations: 'Organisaties',
            implementations: 'Alle implementaties',
            tags: 'Labels',
        },
        options: {
            all_organizations: 'Alle organisaties',
            all_implementations: 'Alle implementaties',
            all_labels: 'Alle labels',
        },
    },
    available: {
        applied_for_fund: {
            title: 'Uw aanvraag is ontvangen.',
            description:
                'De gemeente zal uw verzoek behandelen, dit kan maximaal twee weken duren. Zodra de gemeente uw aanvraag heeft behandeld wordt er een e-mail toegestuurd. Daarnaast kunt u de status ook volgen op het dashboard.',
        },
        error_apply: {
            title: 'U heeft nog geen vestigingen aangemaakt!',
            description: 'U heeft tenminste één vestiging nodig om uw organisatie aan te melden voor',
        },
    },
    status: {
        hold: 'Wachten',
        accepted: 'Geaccepteerd',
        reject: 'Geweigerd',
        stopped: 'Gestopt',
        accepted_only_products: 'Geaccepteerd: alleen uw aanbod',
        accepted_only_specific_products: 'Geaccepteerd: specifiek aanbod',
        pending: 'Uitgenodigd',
        expired: 'Verlopen',
        rejected: 'Geweigerd',
        closed: 'Gesloten',
    },
    tabs: {
        active: 'Actief',
        invitations: 'Uitnodigingen',
        invitations_archived: 'Uitnodigingen archief',
        pending_rejected: 'Aanmeldingen',
        available: 'Beschikbaar',
        archived: 'Archief',
        unsubscriptions: 'Uitgeschreven',
    },
};
