export default {
    title: 'Nieuwe transactie',
    top_up_title: 'Nieuwe top up',
    preview: {
        title: 'Bevestig de transactie',
        description:
            'Deze transactie wordt gecontroleerd en in het volgende betaalbestand meegenomen. De transactie kan daarna niet meer ongedaan gemaakt worden.',
    },
    success: {
        title: 'Transactie is verzonden',
        description: 'De transactie is klaargezet en wordt binnen 3 werkdagen uitbetaald!',
        top_up_title: 'Transactie is verzonden',
        top_up_description: 'De transactie is klaargezet en wordt binnen 3 werkdagen uitbetaald!',
    },
    labels: {
        note: 'Notitie',
        note_shared: 'Deel de notitie met de aanbieder',
        amount: 'Bedrag',
        provider: 'Aanbieder',
        organization: 'Betaal vanuit',
        target: 'Overboeken naar',
        target_iban: 'IBAN-nummer',
        target_name: 'IBAN-naam',
        target_provider_option: 'Aanbieder',
        target_iban_option: 'Bankrekening',
        iban_source: 'Soort',
        reimbursement: 'Declaratie',
        reimbursement_source: 'Declaratie',
        manual_source: 'Handmatig',
    },
    buttons: {
        cancel: 'Annuleren',
        submit: 'Betaal',
        close: 'Bevestigen',
    },
};
